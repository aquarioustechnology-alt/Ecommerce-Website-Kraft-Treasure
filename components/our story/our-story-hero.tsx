...
export function OurStoryHero() {
  return (
    <section className="relative overflow-hidden border-b border-black/10 bg-[linear-gradient(180deg,#ffffff_0%,#f7f1df_18%,#fffdf7_52%,#f4efe4_100%)] pt-10 pb-16 lg:pt-16 lg:pb-24">
      <div aria-hidden className="absolute inset-x-0 top-0 h-px bg-black/10" />
      ...
      <div className="relative mx-auto grid max-w-[1440px] gap-10 px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-12 xl:gap-16">
        <div className="flex flex-col justify-center">
          ...
          <h1 className="mb-5 max-w-[12ch] text-[40px] font-serif leading-[1.1] tracking-tight text-black sm:text-5xl md:text-[54px] lg:text-[62px] xl:text-[70px]">
            A living archive of mountain craft.
          </h1>
        ...
        <div className="relative min-h-[540px]">
          <div className="absolute right-0 top-0 h-[80%] w-[82%] overflow-hidden border border-black/10 bg-white shadow-[0_40px_90px_-45px_rgba(0,0,0,0.35)]">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
              <p className="text-[10px] font-sans uppercase tracking-[0.32em] text-white/75">Fieldwork Portrait</p>
              <p className="mt-2 max-w-xs text-lg font-serif leading-tight">
                Relationships built over time, not transactions closed in a season.
              </p>
            </div>
          </div>

          <div className="absolute left-0 top-[18%] w-[44%] overflow-hidden border border-black/10 bg-[#111111] p-3 text-white shadow-[0_30px_70px_-40px_rgba(0,0,0,0.45)]">
          ...
          <div className="absolute bottom-0 right-[8%] w-[52%] border border-black/10 bg-white/95 p-5 shadow-[0_30px_60px_-40px_rgba(0,0,0,0.3)] backdrop-blur-sm">
            <p className="text-[10px] font-sans uppercase tracking-[0.28em] text-[#D33740]">Curatorial Principle</p>
            <p className="mt-3 text-2xl font-serif leading-tight text-black">
              Cultural context should travel with the craft.
            </p>
            <p className="mt-3 text-sm leading-6 text-black/65">
              That means provenance, artisan attribution, responsible pricing, and storytelling that protects complexity instead of flattening it.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="relative mx-auto mt-12 grid max-w-[1440px] gap-4 px-6 lg:grid-cols-3 lg:px-12">
        {highlights.map((item) => (
          <article
            key={item.title}
            className="border border-black/10 bg-white/70 p-6 backdrop-blur-sm transition-transform duration-300 hover:-translate-y-1"
          >
